...
public class Menu : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        GameObject simObj = GameObject.Find("Simulator");
        sim = simObj.GetComponent<Simulator>();
    ...
    public void SetPopulation_Input(string popsize)
    {
        ...
        if (temp < 1 || temp > 2048) {throw new ArgumentException();}
        else
        {
            pop_last_input = temp;
            population = temp;
    ...
    {
        population = 128 * (int)Math.Pow(2, index);
