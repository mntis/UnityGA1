...
namespace Academy
{
    public class Academy
    {
        ...
        public Academy(int pop, int epa, float mr, float d, Simulator parent)
        {
            POPULATION = pop;
            ENVS_PER_AGENT = epa;
            MUTATION_RATE = mr;
        ...
        public void Setup()
        {
            ...
            for (int i = 0; i < POPULATION; i++)
            {
                ...
                agents[i].SetStrategy(GenerateRandomStrategy());
                agents[i].SetFitness(0);
            }
        
            environment = new Environment();
            environment.EnvSetupBase(NOMS_PER_ENV);
        }
        ...
        public void AgentTrial(int agIndex)
        {
            float agentMeanFitness = 0;

            for (int i = 0; i < ENVS_PER_AGENT; i++)
            {
                environment.EnvSet(epsilon, agents[agIndex].GetStrategy());
                int score = environment.RunEnv();
        ...
        private string GenerateRandomStrategy()
        {
            string result = "";

            for (int i = 0; i < 162; i++)
            {
                result += Random.Range(1, 6).ToString();
            }
